...
function Followers() {
  ...
  const { followers, isLoading } = useUserFollowers(username || '');
  ...
  return (
    <div className="flex flex-col gap-4 p-4 ">
      <PageHeader text="Followers" backTo={`/profile/${username}`} />
      ...
      {followers && followers.length > 0 ? (
        ...
        <span className="flex flex-col items-center justify-center py-12 text-[var(--color-grey-600)]">
          <svg
            width="48"
            height="48"
            fill="none"
          ...
          <span className="text-lg font-medium">{`@${username} doesn’t have any followers`}</span>
